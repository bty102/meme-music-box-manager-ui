...
function InvoiceDetailPage() {
  ...
  const [roomsOfInvoice, setRoomsOfInvoice] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const roomsResponse = await getRoomsOfInvoiceApi(invoiceId);

        const productsResponse = await getProductsOfInvoiceApi(invoiceId);

        setInvoice(invoiceResponse);

        setRoomsOfInvoice(roomsResponse);

        setProductsOfInvoice(productsResponse);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExportPdf = async () => {
    try {
      const pdfBlob = await exportInvoicePdfApi(invoice.id);
      ...
      alert(error.response?.data?.message || "Xuất hóa đơn thất bại");
    }
  };
        ...
        >
          ...
          <Stack spacing={2}>
            <Box>
              ...
              <Typography fontWeight={700}>
                {formatCurrency(invoice?.roomCharge)}
              </Typography>
            </Box>
            ...
            <Box>
              ...
              <Typography fontWeight={700}>{invoice?.vatPercent}%</Typography>
            </Box>
      ...
      >
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {roomsOfInvoice.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>{item.room.roomNumber}</TableCell>

                  <TableCell>{formatDateTime(item.checkInAt)}</TableCell>

                  <TableCell>{formatDateTime(item.checkOutAt)}</TableCell>

                  <TableCell>{item.durationHours}</TableCell>

                  <TableCell>{formatCurrency(item.roomCharge)}</TableCell>

                  <TableCell>
                    <Chip
                      label={item.isTransferred ? "Đã chuyển" : "Hiện tại"}
                      color={item.isTransferred ? "warning" : "success"}
                      size="small"
                    />
                  </TableCell>
                </TableRow>
